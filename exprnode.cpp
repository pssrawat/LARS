...
void id_node::print_node (stringstream &out, vector<string> &initialized_labels, vector<string> &iters, bool perform_load, bool is_lhs) {
	if (print_intrinsics && find (initialized_labels.begin(), initialized_labels.end(), label) == initialized_labels.end ()) {
		if (type == DOUBLE) out << "_mm" << to_string(vsize) << "_set1_pd (" << label << ")";
		else if (type == FLOAT) out << "_mm" << to_string(vsize) << "_set1_ps (" << label << ")";
		else out << "_mm" << to_string(vsize) << "_set1_epi64 (" << label << ")";
	}
	//else out << label;
	else out << name;
}
...
void id_node::print_initializations (stringstream &header_output, vector<string> &initialized_labels, vector<string> iters, bool perform_load, bool is_lhs) {
	if (!print_intrinsics) {
		...
		if (find (initialized_labels.begin(), initialized_labels.end(), label) == initialized_labels.end ()) {
			if (type == DOUBLE) header_output << "__m" << to_string(vsize) << "d " << label;
			else if (type == FLOAT) header_output << "__m" << to_string(vsize) << " " << label;
			else  header_output << "__m" << to_string(vsize) << "i " << label;
			header_output << ";\n";
			initialized_labels.push_back (label);
		}
	}
...
expr_node *id_node::unroll_expr (string s, int val, vector<string> coefficients, map<string,int> &scalar_count, bool is_lhs) {
	...
	if (scalar_count[name] == 0)
		temp1 = new id_node (name, type, nested, label, vsize, gen_fma, print_intrinsics);
	else {
		string new_name = name + "_" + to_string (scalar_count[name]) + "_";
		temp1 = new id_node (new_name, type, nested, label, vsize, gen_fma, print_intrinsics); 
	}
	if (name.compare (s) == 0 && val!=0) {
		expr_node *temp2 = new datatype_node<int> (val, INT, vsize, gen_fma, print_intrinsics);
		return new binary_node (T_PLUS, temp1, temp2, name, type, nested, vsize, gen_fma, print_intrinsics);	
	}
	else return temp1;
}

expr_node *id_node::deep_copy (void) {
	return new id_node (name, type, nested, label, vsize, gen_fma, print_intrinsics);
}
...
void id_node::decompose_node (vector<tuple<expr_node*, expr_node*, STMT_OP>> &tstmt, vector<tuple<expr_node*, expr_node*, STMT_OP>> &init, vector<expr_node*> &temp_vars, expr_node *alhs, STMT_OP cur_op, int &id, DATA_TYPE gdata_type, bool &local_assigned, bool &global_assigned, bool flip) {
	...
	if (!global_assigned && cur_op != ST_EQ)
		init.push_back (make_tuple (alhs, new datatype_node<int> (get_init_val (cur_op), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
...
expr_node *uminus_node::unroll_expr (string s, int val, vector<string> coefficients, map<string,int> &scalar_count, bool is_lhs) {
	...
	return new uminus_node (temp, name, type, nested, vsize, gen_fma, print_intrinsics);	
}
...
void uminus_node::decompose_node (vector<tuple<expr_node*, expr_node*, STMT_OP>> &tstmt, vector<tuple<expr_node*, expr_node*, STMT_OP>> &init, vector<expr_node*> &temp_vars, expr_node *alhs, STMT_OP cur_op, int &id, DATA_TYPE gdata_type, bool &local_assigned, bool &global_assigned, bool flip) {
	...
	if (base_expr->is_id_type (gdata_type) || base_expr->is_data_type (gdata_type) || base_expr->is_shiftvec_type (gdata_type)) {
		...
		expr_node *temp1 = new id_node (name_t, vsize, gen_fma, print_intrinsics);
		...
		expr_node *new_rhs = new uminus_node (temp1, vsize, gen_fma, print_intrinsics);
		if (!local_assigned) cur_op = acc_start_op (cur_op);
		if (!global_assigned && cur_op != ST_EQ) 
			init.push_back (make_tuple (alhs, new datatype_node<int> (get_init_val (cur_op), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
...
void binary_node::set_codegen_parameters (int v, bool g, bool p) {
	vsize = v;
        gen_fma = g;
        print_intrinsics = p;
	lhs->set_codegen_parameters (v, g, p);
...
void binary_node::print_node (stringstream &out) {
	if (!print_intrinsics) {
		...
		type = infer_data_type (lhs->get_type (), rhs->get_type ());
		out << print_operator (op, type, vsize);
		out << " (";
		lhs->print_node (out);
		out << ", ";
		rhs->print_node (out);
...
expr_node *binary_node::unroll_expr (string s, int val, vector<string> coefficients, map<string,int> &scalar_count, bool is_lhs) {
	...
	return new binary_node (op, temp1, temp2, name, type, nested, vsize, gen_fma, print_intrinsics);
}
...
void binary_node::decompose_node (vector<tuple<expr_node*, expr_node*, STMT_OP>> &tstmt, vector<tuple<expr_node*, expr_node*, STMT_OP>> &init, vector<expr_node*> &temp_vars, expr_node *alhs, STMT_OP cur_op, int &id, DATA_TYPE gdata_type, bool &local_assigned, bool &global_assigned, bool flip) {
	...
	if (op == T_PLUS || op == T_MINUS) {
		...
		if (lhs->is_data_type (gdata_type)) {
			...
			expr_node *temp2 = new id_node (name_t2, vsize, gen_fma, print_intrinsics);
			...
			expr_node *new_rhs = new binary_node (op, lhs, temp2, vsize, gen_fma, print_intrinsics);
			if (!local_assigned) cur_op = acc_start_op (cur_op);
			cur_op = get_cur_op (cur_op, flip);
			if (!global_assigned && cur_op != ST_EQ)
				init.push_back (make_tuple (alhs, new datatype_node<int> (get_init_val (cur_op), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
			...
			expr_node *temp1 = new id_node (name_t1, vsize, gen_fma, print_intrinsics);
			...
			expr_node *new_rhs = new binary_node (op, temp1, rhs, vsize, gen_fma, print_intrinsics);
			if (!local_assigned) cur_op = acc_start_op (cur_op);
			cur_op = get_cur_op (cur_op, flip);
			if (!global_assigned && cur_op != ST_EQ) 
				init.push_back (make_tuple (alhs, new datatype_node<int> (get_init_val (cur_op), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
			...
			if (cur_op == convert_op_to_stmt_op (op)) {
				...
				cur_op = get_cur_op (cur_op, flip);
				if (cur_op != ST_EQ) 
					init.push_back (make_tuple (temp1, new datatype_node<int> (get_init_val (convert_op_to_stmt_op (op)), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
			...
			if (cur_op == convert_op_to_stmt_op (op)) {
				...
				string name_t1 = "_t_" + to_string (id++) + "_";
				expr_node *new_rhs = new id_node (name_t1, vsize, gen_fma, print_intrinsics);
				string name_t2 = "_t_" + to_string (id++) + "_";
				expr_node *temp2 = new id_node (name_t2, vsize, gen_fma, print_intrinsics);
				bool nested_local_assigned2 = false;
				bool nested_global_assigned2 = false;
				...
				cur_op = get_cur_op (cur_op, flip);
				if (cur_op != ST_EQ) 
					init.push_back (make_tuple (new_rhs, new datatype_node<int> (get_init_val (convert_op_to_stmt_op (op)), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
			...
			if (cur_op == convert_op_to_stmt_op (op)) {
				...
				string name_t2 = "_t_" + to_string (id++) + "_";
				expr_node *new_rhs = new id_node (name_t2, vsize, gen_fma, print_intrinsics);
				string name_t1 = "_t_" + to_string (id++) + "_";
				expr_node *temp1 = new id_node (name_t1, vsize, gen_fma, print_intrinsics);
				bool nested_local_assigned1 = false;
				bool nested_global_assigned1 = false;
			...
			if (cur_op == convert_op_to_stmt_op (op)) {
				...
				string name_t1 = "_t_" + to_string (id++) + "_";
				string name_t2 = "_t_" + to_string (id++) + "_";
				expr_node *new_rhs = new id_node (name_t0, vsize, gen_fma, print_intrinsics);
				expr_node *temp1 = new id_node (name_t1, vsize, gen_fma, print_intrinsics);
				expr_node *temp2 = new id_node (name_t2, vsize, gen_fma, print_intrinsics);
		...
		if ((lhs->is_id_type (gdata_type) || lhs->is_data_type (gdata_type) || lhs->is_shiftvec_type (gdata_type)) &&
			...
			lhs->decompose_node (tstmt, init, temp_vars, temp1, ST_EQ, id, gdata_type, nested_local_assigned1, nested_global_assigned1, nested_flip1);
			rhs->decompose_node (tstmt, init, temp_vars, temp2, ST_EQ, id, gdata_type, nested_local_assigned2, nested_global_assigned2, nested_flip2);
			expr_node *new_rhs = new binary_node (op, temp1, temp2, vsize, gen_fma, print_intrinsics);
			if (!local_assigned) cur_op = acc_start_op (cur_op);
			cur_op = get_cur_op (cur_op, flip);
			if (!global_assigned && cur_op != ST_EQ) 
				init.push_back (make_tuple (alhs, new datatype_node<int> (get_init_val (cur_op), INT, vsize, gen_fma, print_intrinsics), ST_EQ));
...
void shiftvec_node::print_node (stringstream &out, vector<string> &initialized_labels, vector<string> &iters, bool perform_load, bool is_lhs) {
	if (perform_load || is_lhs || (find (initialized_labels.begin(), initialized_labels.end(), label) != initialized_labels.end())) 
		...
		if (!print_intrinsics) 
			...
			if (splat) {
				if (type == DOUBLE) out << "_mm" << to_string(vsize) << "_set1_pd (" << print_array () << ")";
				else if (type == FLOAT) out << "_mm" << to_string(vsize) << "_set1_ps (" << print_array () << ")";
				else out << "_mm" << to_string(vsize) << "_set1_epi64 (" << print_array () << ")";
			}
			else {
				if (type == DOUBLE) out << "_mm" << to_string(vsize) << "_loadu_pd (&" << print_array () << ")";
				else if (type == FLOAT) out << "_mm" << to_string(vsize) << "_loadu_ps (&" << print_array () << ")";
				else out << "_mm" << to_string(vsize) << "_loadu_epi64 (&" << print_array () << ")";
			}
		}
	}
}
	...
	if (!print_intrinsics) {
		...
		if (find (initialized_labels.begin(), initialized_labels.end(), label) == initialized_labels.end ()) {
			...
			if (perform_load) {
				...
				header_output << " = ";
				if (splat) {
					if (type == DOUBLE) header_output << "_mm" << to_string(vsize) << "_set1_pd (" << print_array () << ")";
					else if (type == FLOAT) header_output << "_mm" << to_string(vsize) << "_set1_ps (" << print_array () << ")";
					else header_output << "_mm" << to_string(vsize) << "_set_epi64 (" << print_array () << ")";
				}
				else {
					if (type == DOUBLE) header_output << "_mm" << to_string(vsize) << "_loadu_pd (&" << print_array () << ")";
					else if (type == FLOAT) header_output << "_mm" << to_string(vsize) << "_loadu_ps (&" << print_array () << ")";
					else header_output << "_mm" << to_string(vsize) << "_loadu_epi64 (&" << print_array () << ")";
				}
			}
			header_output << ";\n";
			initialized_labels.push_back (label);
...
expr_node *shiftvec_node::unroll_expr (string s, int val, vector<string> coefficients, map<string,int> &scalar_count, bool is_lhs) {
	shiftvec_node *ret = new shiftvec_node (name, vsize, gen_fma, print_intrinsics);
...
expr_node *shiftvec_node::deep_copy (void) {
	shiftvec_node *ret = new shiftvec_node (name, type, nested, label, vsize, gen_fma, print_intrinsics);
	for (vector<expr_node*>::const_iterator i=indices.begin(); i!=indices.end(); i++) {
...
void function_node::set_codegen_parameters (int v, bool g, bool p) {
	vsize = v;
        gen_fma = g;
        print_intrinsics = p;
