...
void stmtnode::print_statement (map<string, string> &reg_map, map<string, expr_node*> &label_to_node_map, map<string,int> &first_load, map<string,int> &last_write, vector<string> &printed_regs, stringstream &output, int vsize) {
		...
		if (acc_vars.find (*it) != acc_vars.end ()) {
			...
			if (get<0>(index_value) > 0) {
				...
				for (int j=1; j<=get<0>(index_value); j++) {
					...
					stmtnode *new_stmt = new stmtnode (ST_PLUSEQ, lhs, new id_node (new_label, gdata_type, vsize, gen_fma, print_intrinsics), vsize, gen_fma, print_intrinsics);
...
void funcdefn::set_codegen_parameters (int v, bool g, bool p) {
	vsize = v;
	gen_fma = g;
	print_intrinsics = p;
	vector<stmtnode*> stmts = stmt_list->get_stmt_list ();
	for (vector<stmtnode*>::iterator i=stmts.begin(); i!=stmts.end(); i++) {
		(*i)->set_vect_size (v);
		(*i)->set_gen_fma (g);
		(*i)->set_print_intrinsics (p);
		expr_node *lhs = (*i)->get_lhs_expr ();
		expr_node *rhs = (*i)->get_rhs_expr ();
		lhs->set_codegen_parameters (vsize, gen_fma, print_intrinsics);
		rhs->set_codegen_parameters (vsize, gen_fma, print_intrinsics);
	}
}
...
void funcdefn::compute_order_metric (void) {
	...
	if (!ordered_labels.empty ()) {
		...
		if (FIRST_LEVEL && !SECOND_LEVEL)
			...
			if (sort_function.compare ("v0") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b0);
			if (sort_function.compare ("v0-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b0_ilp);
			if (sort_function.compare ("v1") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b1);
			if (sort_function.compare ("v1-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b1_ilp);
			if (sort_function.compare ("v2") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b2);
			if (sort_function.compare ("v2-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b2_ilp);
			if (sort_function.compare ("v3") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b3);
			if (sort_function.compare ("v3-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b3_ilp);
			if (sort_function.compare ("v4") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b4);
			if (sort_function.compare ("v4-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b4_ilp);
			if (sort_function.compare ("v5") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b5);
			if (sort_function.compare ("v5-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b5_ilp);
			if (sort_function.compare ("v6") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b6);
			if (sort_function.compare ("v6-ilp") == 0)
				sort (order_metric.begin(), order_metric.end(), sort_order_metric_b6_ilp);
		}
	}
}
...
void funcdefn::analyze_statements (stringstream &output) {
	...
	if (DEBUG) printf ("ACCUMULATION SLC VERSION TAKES %d REGISTERS\n", reg_count);
	...
	for (deque<stmtnode*>::iterator it=fireable_stmts.begin(); it!=fireable_stmts.end(); it++) {
		stmtnode *fired_node = new stmtnode ((*it)->get_op_type (), (*it)->get_lhs_expr (), (*it)->get_rhs_expr (), vsize, gen_fma, print_intrinsics);
...
void funcdefn::print_reordered_stmts (stringstream &output, map<string,stmtnode*> &last_write, deque<stmtnode*> &fireable_stmts, PRINT_OPTION print) {
	...
	if (print == INIT_ASSIGN && !print_intrinsics) {
		...
		while (fireable_stmts.size () != 0) {
			...
			for (vector<stmtnode*>::iterator i=init.begin(); i!=init.end();) {
				if ((*i)->get_lhs_expr () == lhs) {
					header_output << (*i)->print_statement (tmp_output, initialized_labels, iters, vsize);
					i = init.erase (i);
					break;
				}
				else ++i; 
			}
			header_output << fired_node->print_statement (tmp_output, initialized_labels, iters, vsize);
		...
		while (fireable_stmts.size () != 0) {
			...
			for (vector<string>::iterator j=l_labels.begin(); j!=l_labels.end(); j++) {
				if (last_write.find (*j) != last_write.end () && last_write[*j] == fired_node) {
					...
					if (lhs_type == DOUBLE) tmp_output << "_mm" << to_string(vsize) << "_storeu_pd (&";
					else if (lhs_type == FLOAT) tmp_output << "_mm" << to_string(vsize) << "_storeu_ps (&";
					else tmp_output << "_mm" << to_string(vsize) << "_storeu_epi64 (&";
					tmp_output << dynamic_cast<shiftvec_node*>(fired_node->get_lhs_expr())->print_array () << ", " << *j << ");\n";
				}
			}
		}
		...
		while (fireable_stmts.size () != 0) {
			...
			if (init_found) {
				if (fired_node->get_op_type () == ST_MINUSEQ)
					fired_node->set_rhs_expr (new uminus_node (fired_node->get_rhs_expr(), vsize, gen_fma, print_intrinsics));
...
void start_node:: push_iterator (vector<string> &it) {
	iters = it;
}
