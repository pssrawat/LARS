...
#pragma omp for private(i)
			for(j = 1; j < N-1; j+=8) {
				for(i = 1; i < N-1; i+=1) {
#pragma begin stencil2 unroll j=8,i=1 print-intrinsics false acc-size 1
					node_mass_post[j][i]=0.25*(density1[j-1][i]*post_vol[j-1][i] +density1[j][i]*post_vol[j][i]  +density1[j-1][i-1]*post_vol[j-1][i-1] +density1[j][i-1]*post_vol[j][i-1]);
					node_mass_pre[j][i]=node_mass_post[j][i]-node_flux[j][i-1]+node_flux[j][i];
#pragma end stencil2
				}
...
#pragma omp for private(i)
			for(j = 1; j < N-1; j+=8) {
				for(i = 1; i < N-1; i+=1) {
#pragma begin stencil3 unroll j=8,i=1 print-intrinsics false acc-size 1
...
#pragma omp for private(i)
			for(j = 1; j < N-1; j+=8) {
				for(i = 1; i < N-1; i+=1) {
#pragma begin stencil4 unroll j=8,i=1 print-intrinsics false acc-size 1
