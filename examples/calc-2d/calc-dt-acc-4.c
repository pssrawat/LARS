...
#pragma omp for private(i)
			for (j = 1; j < N-1; j+=4) {
				for (i = 1; i < N-1; i+=1) {
#pragma begin stencil1 unroll j=1,i=1 print-intrinsics false acc-size 1
					...
					cc=soundspeed[j+1][i]*soundspeed[j+1][i];
					cc=cc+2.0*viscosity_a[j+1][i]/density0[j+1][i];
					dtct=dtc_safe*celldx[i]*celldy[j+1]/sqrt(cc);
					dv1=(xvel0[j+1][i]+xvel0[j+1+1][i])*xarea[j+1][i];
					dv2=(xvel0[j+1][i+1]+xvel0[j+1+1][i+1])*xarea[j+1][i+1];
					div=dv2-dv1;
					dtut=dtu_safe*2.0*volume[j+1][i]/(dv1*dv2*volume[j+1][i]);

					dv1=(yvel0[j+1][i]+yvel0[j+1][i+1])*yarea[j+1][i];
					dv2=(yvel0[j+1+1][i]+yvel0[j+1+1][i+1])*yarea[j+1+1][i];
					div=div+dv2-dv1;
					dtvt=dtv_safe*2.0*volume[j+1][i]/(dv1*dv2*volume[j+1][i]);
					div=div/(2.0*volume[j+1][i]);
					dtdivt=dtdiv_safe*(-1.0/div);
					dt_min[j+1][i]=dtct*dtut*dtvt*dtdivt;


					cc=soundspeed[j+2][i]*soundspeed[j+2][i];
					cc=cc+2.0*viscosity_a[j+2][i]/density0[j+2][i];
					dtct=dtc_safe*celldx[i]*celldy[j+2]/sqrt(cc);
					dv1=(xvel0[j+2][i]+xvel0[j+2+1][i])*xarea[j+2][i];
					dv2=(xvel0[j+2][i+1]+xvel0[j+2+1][i+1])*xarea[j+2][i+1];
					div=dv2-dv1;
					dtut=dtu_safe*2.0*volume[j+2][i]/(dv1*dv2*volume[j+2][i]);

					dv1=(yvel0[j+2][i]+yvel0[j+2][i+1])*yarea[j+2][i];
					dv2=(yvel0[j+2+1][i]+yvel0[j+2+1][i+1])*yarea[j+2+1][i];
					div=div+dv2-dv1;
					dtvt=dtv_safe*2.0*volume[j+2][i]/(dv1*dv2*volume[j+2][i]);
					div=div/(2.0*volume[j+2][i]);
					dtdivt=dtdiv_safe*(-1.0/div);
					dt_min[j+2][i]=dtct*dtut*dtvt*dtdivt;


					cc=soundspeed[j+3][i]*soundspeed[j+3][i];
					cc=cc+2.0*viscosity_a[j+3][i]/density0[j+3][i];
					dtct=dtc_safe*celldx[i]*celldy[j+3]/sqrt(cc);
					dv1=(xvel0[j+3][i]+xvel0[j+3+1][i])*xarea[j+3][i];
					dv2=(xvel0[j+3][i+1]+xvel0[j+3+1][i+1])*xarea[j+3][i+1];
					div=dv2-dv1;
					dtut=dtu_safe*2.0*volume[j+3][i]/(dv1*dv2*volume[j+3][i]);

					dv1=(yvel0[j+3][i]+yvel0[j+3][i+1])*yarea[j+3][i];
					dv2=(yvel0[j+3+1][i]+yvel0[j+3+1][i+1])*yarea[j+3+1][i];
					div=div+dv2-dv1;
					dtvt=dtv_safe*2.0*volume[j+3][i]/(dv1*dv2*volume[j+3][i]);
					div=div/(2.0*volume[j+3][i]);
					dtdivt=dtdiv_safe*(-1.0/div);
					dt_min[j+3][i]=dtct*dtut*dtvt*dtdivt;
#pragma end stencil1
				}
			}
		}
