...
int main (void) {
	cacheflush ();
	int N = 4098;
	double (*xvel1_opt)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);
	double (*yvel1_opt)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);
	double (*zvel1_opt)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);

	double (*xvel1)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);
	double (*yvel1)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);
	double (*zvel1)[4098] = (double (*)[4098]) getZero2DArray (4098, 4098);
...
#pragma clang loop vectorize (enable) interleave(enable)
	...
	double error = checkError2D (N, 0, (double*)xvel1_opt, (double*)xvel1, 1, N-1, 1, N-1);
	if (error > TOLERANCE)
		printf ("error %e\n", error);
	error = checkError2D (N, 0, (double*)yvel1_opt, (double*)yvel1, 1, N-1, 1, N-1);
	if (error > TOLERANCE)
		printf ("error %e\n", error);
	error = checkError2D (N, 0, (double*)zvel1_opt, (double*)zvel1, 1, N-1, 1, N-1);
        if (error > TOLERANCE)
		printf ("error %e\n", error);
}
