...
#pragma begin stencil1 unroll j=1,i=1 print-intrinsics false acc-size 1
					...
					xvel1[j][i]-=stepbymass_s*(xarea[j][i]*(viscosity[j][i]-viscosity[j][i-1]) +xarea[j-1][i]*(viscosity[j-1][i]-viscosity[j-1][i-1]));
					yvel1[j][i]-=stepbymass_s*(yarea[j][i]*(viscosity[j][i]-viscosity[j-1][i]) +yarea[j][i-1]*(viscosity[j][i-1]-viscosity[j-1][i-1]));
#pragma end stencil1
				}
			}
		}
