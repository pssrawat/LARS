...
extern void j2d81pt_opt (double*, double*, double*, int);
...
#pragma clang loop vectorize (enable) interleave(enable)
	...
	end_time = rtclock ();
        ...
        j2d81pt_opt ((double*)in, (double*)out, (double*)c, N);

        double error = checkError2D (N, 0, (double*)out, (double*) out_ref, 0, N-8, 0, N-8);
        if (error > TOLERANCE)
                printf ("error %e\n", error);
}
