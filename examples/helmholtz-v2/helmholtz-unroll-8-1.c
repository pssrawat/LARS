...
#pragma GCC ivdep
					for (i = 1; i < N-1; i++) {
						...
						out[k+4][j][i] = a*alpha[k+4][j][i]*x[k+4][j][i] - b*h2inv*(
								beta_i[k+4][j][i+1] * (x[k+4][j][i+1] - x[k+4][j][i]) + 
								beta_i[k+4][j][i] * (x[k+4][j][i-1] - x[k+4][j][i]) + 
								beta_j[k+4][j+1][i] * (x[k+4][j+1][i] - x[k+4][j][i]) + 
								beta_j[k+4][j][i] * (x[k+4][j-1][i] - x[k+4][j][i]) + 
								beta_k[k+4+1][j][i] * (x[k+4+1][j][i] - x[k+4][j][i]) + 
								beta_k[k+4][j][i] * (x[k+4-1][j][i] - x[k+4][j][i]));

						out[k+5][j][i] = a*alpha[k+5][j][i]*x[k+5][j][i] - b*h2inv*(
								beta_i[k+5][j][i+1] * (x[k+5][j][i+1] - x[k+5][j][i]) + 
								beta_i[k+5][j][i] * (x[k+5][j][i-1] - x[k+5][j][i]) + 
								beta_j[k+5][j+1][i] * (x[k+5][j+1][i] - x[k+5][j][i]) + 
								beta_j[k+5][j][i] * (x[k+5][j-1][i] - x[k+5][j][i]) + 
								beta_k[k+5+1][j][i] * (x[k+5+1][j][i] - x[k+5][j][i]) + 
								beta_k[k+5][j][i] * (x[k+5-1][j][i] - x[k+5][j][i]));

						out[k+6][j][i] = a*alpha[k+6][j][i]*x[k+6][j][i] - b*h2inv*(
								beta_i[k+6][j][i+1] * (x[k+6][j][i+1] - x[k+6][j][i]) + 
								beta_i[k+6][j][i] * (x[k+6][j][i-1] - x[k+6][j][i]) + 
								beta_j[k+6][j+1][i] * (x[k+6][j+1][i] - x[k+6][j][i]) + 
								beta_j[k+6][j][i] * (x[k+6][j-1][i] - x[k+6][j][i]) + 
								beta_k[k+6+1][j][i] * (x[k+6+1][j][i] - x[k+6][j][i]) + 
								beta_k[k+6][j][i] * (x[k+6-1][j][i] - x[k+6][j][i]));

						out[k+7][j][i] = a*alpha[k+7][j][i]*x[k+7][j][i] - b*h2inv*(
								beta_i[k+7][j][i+1] * (x[k+7][j][i+1] - x[k+7][j][i]) + 
								beta_i[k+7][j][i] * (x[k+7][j][i-1] - x[k+7][j][i]) + 
								beta_j[k+7][j+1][i] * (x[k+7][j+1][i] - x[k+7][j][i]) + 
								beta_j[k+7][j][i] * (x[k+7][j-1][i] - x[k+7][j][i]) + 
								beta_k[k+7+1][j][i] * (x[k+7+1][j][i] - x[k+7][j][i]) + 
								beta_k[k+7][j][i] * (x[k+7-1][j][i] - x[k+7][j][i]));
