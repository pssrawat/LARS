...
inline std::string print_operator (OP_TYPE op, DATA_TYPE type, int vsize) {
	std::string str;
	std::string tail = (type == DOUBLE) ? "pd" : "ps";
	switch (op) {
		case T_PLUS:
			str = std::string ("_mm" + std::to_string(vsize) + "_add_") + tail;
			break;
		case T_MINUS:
			str = std::string ("_mm" + std::to_string(vsize) + "_sub_") + tail;
			break;
		case T_MULT:
			str = std::string ("_mm" + std::to_string(vsize) + "_mul_") + tail;
			break;
		case T_DIV:
			str = std::string ("_mm" + std::to_string(vsize) + "_div_") + tail;
			break;
		case T_OR:
			str = std::string ("_mm" + std::to_string(vsize) + "_or_") + tail;
			break;
		case T_AND:
			str = std::string ("_mm" + std::to_string(vsize) + "_and_") + tail;
			break;
		default:
